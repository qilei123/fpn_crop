...
from bbox.bbox_transform import bbox_overlaps, bbox_transform, bbox_centerIn_py,bbox_overlaps_centerIns
...
def assign_pyramid_anchor_crop(feat_shapes, gt_boxes, im_info, cfg, feat_strides=(4, 8, 16, 32, 64),
                          scales=(8,), ratios=(0.5, 1, 2), allowed_border=0, balance_scale_bg=False,):
    """
    assign ground truth boxes to anchor positions
    :param feat_shapes: infer output shape
    ...
    for feat_id in range(len(feat_strides)):
        ...
        t_all_anchors = t_all_anchors.reshape((K * A, 4))
        t_total_anchors = int(K * A)

        total_anchors = int(t_total_anchors * channel_num)
        all_anchors = np.zeros((total_anchors,5),dtype=t_all_anchors.dtype)
    ...
    if balance_scale_bg:
        ...
        if len(bg_inds) > num_bg:
            ...
            fpn_labels[disable_inds] = -1
            #print "len(disable_inds):"+str(len(disable_inds))
    fpn_bbox_targets = np.zeros((len(fpn_anchors), 5), dtype=np.float32)
    if gt_boxes.size > 0:
        fpn_bbox_targets[fpn_labels >= 1, :] = bbox_transform(fpn_anchors[fpn_labels >= 1, :], gt_boxes[argmax_overlaps[fpn_labels >= 1], :5])
        # fpn_bbox_targets[:] = bbox_transform(fpn_anchors, gt_boxes[argmax_overlaps, :4])
    # fpn_bbox_targets = (fpn_bbox_targets - np.array(cfg.TRAIN.BBOX_MEANS)) / np.array(cfg.TRAIN.BBOX_STDS)
    fpn_bbox_weights = np.zeros((len(fpn_anchors), 4), dtype=np.float32)
