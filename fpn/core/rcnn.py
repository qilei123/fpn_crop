...
from bbox.bbox_transform import bbox_overlaps, bbox_transform,bbox_overlaps_centerIns
...
def sample_rois_crop(rois, fg_rois_per_image, rois_per_image, num_classes, cfg,
                labels=None, overlaps=None, bbox_targets=None, gt_boxes=None):
    """
    generate random sample of ROIs comprising foreground and background examples
    :param rois: all_rois [n, 4]; e2e: [n, 5] with batch_index
    ...
    """
    if labels is None:
        overlaps,centerin_overlaps = bbox_overlaps_centerIns(rois[:, 1:].astype(np.float), gt_boxes[:, :5].astype(np.float))
        gt_assignment = overlaps.argmax(axis=1)
        overlaps = overlaps.max(axis=1)
