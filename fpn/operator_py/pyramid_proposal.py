...
class PyramidProposalOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for s in self._feat_stride:
            ...
            channels = np.ones((total_anchors,1),dtype=t_anchors.dtype)
            for channel_i in range(9):
                anchors[int(channel_i*K*A):int((channel_i+1)*K*A),:4] =t_anchors[:,:]
                channels[int(channel_i*K*A):int((channel_i+1)*K*A),:] = channels[int(channel_i*K*A):int((channel_i+1)*K*A),:]*channel_i            
